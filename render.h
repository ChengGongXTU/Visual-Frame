...
void RenderObj(TriangleMesh &mesh, Camera &camera, PointLight &pl) {
	...
	for (int i = 0; i < mesh.triNum; i++) {
		...
		Triangle t = Triangle(Vertex(mesh.p[mesh.vi[3 * i]]), Vertex(mesh.p[mesh.vi[3 * i + 1]]), Vertex(mesh.p[mesh.vi[3 * i + 2]]));
		t.n = mesh.Nor(i);
		t.va.r = t.vb.r = t.vc.r = mesh.uv[3 * i];
		t.va.g = t.vb.g = t.vc.g = mesh.uv[3 * i+1];
		t.va.b = t.vb.b = t.vc.b = mesh.uv[3 * i + 2];
		t.va.pr = t.vb.pr = t.vc.pr = 4.f;
		t.va.pg = t.vb.pg = t.vc.pg = 4.f;
		t.va.pb = t.vb.pb = t.vc.pb = 4.f;
